...
void child(void)
{
	...
	uint64_t child_pa_after_write = get_physical_address((uint64_t)heap_private_buf);
	printf("[CHILD] Physical address of heap_private_buf after  write: 0x%llx\n", (unsigned long long)child_pa_after_write);
	...
	printf("[CHILD] Physical address of heap_shared_buf after  write: 0x%llx\n", (unsigned long long)child_shared_pa);
    ...
    printf("\n[CHILD] Virtual Memory Map of Child process AFTER mprotect (PID: %d):\n", getpid());
	show_maps();
    ...
    if (munmap(heap_private_buf, buffer_size)==-1) die("munmap heap_private_buf child");
	if (munmap(heap_shared_buf, buffer_size)==-1)  die("munmap heap_shared_buf child");
	if (munmap(file_shared_buf, buffer_size)==-1)  die("munmap file_shared_buf child");
}
...
void parent(pid_t child_pid)
{
    ...
    if (munmap(heap_private_buf, buffer_size)==-1) die("munmap heap_private_buf parent");
	if (munmap(heap_shared_buf, buffer_size)==-1)  die("munmap heap_shared_buf parent");
	if (munmap(file_shared_buf, buffer_size)==-1)  die("munmap file_shared_buf parent");
